...
public class ConnectFourGame extends MiniGame
{
	...
	private int spaces = ROWS * COLUMNS; //總格數 = 列 * 行
	...
	private void updateBoardString(char symbol, int row, int column)
	{
		boardBuilder.setCharAt(4 + (CHARACTERS_IN_A_ROW << 1) + //首先排除掉前面的```\n以及上面那兩列 (用 << 1 取代 * 2)
				...
				4 * column + 1 //每一項都是前一項 + 1 這個等差數列是從1開始的 公差是4 所以就不用 << 2 了
				, symbol);
	}
	...
	public boolean aiPlace()
	{
		...
		//place方法回傳即將要落子的那個橫列
		return isWon(AI_PLACE, place(AI_PLACE, column), column); //回傳是否勝利
	}
	...
	private int place(char symbol, int column)
	{
		...
		for (; row < ROWS; row++)
			if (board[row][column] != EMPTY) //找到第一個不是空的
				break; //說明它前一個一定是空的
		row--; //前一個是空的
		board[row][column] = symbol; //找到之後放棋子
		updateBoardString(symbol, row, column); //更新棋盤字串
		recordBuilder.append(boardBuilder.substring(3, boardBuilder.length() - 3)) //去除前後的```
				.append('\n');
		return row;
	}
	...
	private boolean isWon(char symbol, int row, int column)
	{
		...
		// \
		symbolCount = 0;
		int toSubtract = Math.min(row, column); //將r和c歸位到左上角 直到碰到其中一面的邊界為止
		for (r = row - toSubtract, c = column - toSubtract, rowMax = Math.min(ROWS, row + WIN_COUNT), columnMax = Math.min(COLUMNS, column + WIN_COUNT);
		     r < rowMax && c < columnMax; r++, c++)
		{
			if (board[r][c] == symbol)
			{
				symbolCount++;
		...
		// /
		symbolCount = 0;
		int toAdd = Math.min(row, COLUMNS - column - 1); //將r和c歸位到右上角 直到碰到其中一面的邊界為止
		for (r = row - toAdd, c = column + toAdd, rowMax = Math.min(ROWS, row + WIN_COUNT), columnMax = Math.max(-1, column - WIN_COUNT);
		     r < rowMax && c > columnMax; r++, c--)
		{
			if (board[r][c] == symbol)
			{
