...
public class ConnectFourCommand extends HasSubcommands
{
	...
	private static class PlaySubCommand implements ICommand
	{
		...
		public void commandProcess(SlashCommandInteractionEvent event)
		{
			...
			{
				event.reply("connect_four.must_be_in_range").setEphemeral(true).queue();
				return;
			}

			if (connectFour.isFull(column)) //直行已經滿了卻還是放棋子
			{
				event.reply("connect_four.can_t_put").setEphemeral(true).queue();
				return;
			}

			if (connectFour.humanPlace(column)) //如果玩家贏了
			{
				event.reply("connect_four.win" + connectFour.getBoard()).queue();
				games.remove(userID);
				return;
			}
			...
			{
				event.reply("connect_four.lose" + connectFour.getBoard()).queue();
				games.remove(userID);
				return;
			}
			...
			if (connectFour.isTie()) //如果平手
			{
				...
				event.reply("connect_four.tie" + connectFour.getBoard()).queue();
				games.remove(userID);
				return;
			}

			event.reply("connect_four.your_move" + playerMove + "\nconnect_four.bot_s_move" + connectFour.getBoard() + "\n</connect_four play:1142380307509690458>").setEphemeral(true).queue();
		}
	}
}
